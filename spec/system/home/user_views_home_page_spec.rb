...
describe 'Usuário visita a home page' do
  ...
  it 'e não existem posts de usuário que ele segue' do
    ...
    gabriel.posts.create(title: 'Como fazer uma app Vue', content: 'Não esqueça de usar o app.mount')
    gabriel.posts.create(title: 'Como fazer uma app React', content: 'Não esqueça de usar o app.mount')
    third_post = gabriel.posts.create(title: 'Aprenda Ruby on Rails em 1 dia', content: 'Não esqueça...')
    fourth_post = gabriel.posts.create(title: 'Rails new', content: 'Não esqueça de usar o app.mount')
    fifth_post = gabriel.posts.create(title: 'Boas práticas em ruby', content: 'Não esqueça de usar o app.mount')
    ...
    expect(page).to_not have_content 'Como fazer uma app Vue'
  ...
  it 'e não segue nenhum usuário' do
    follower = create(:user, email: 'usuario@email.com', full_name: 'Usuário', citizen_id_number: '56275577029')

    login_as follower
    visit root_path

    expect(page).to have_content 'Você ainda não segue nenhum usuário.'
    expect(page).to have_content 'Para ver o conteúdo de seu interesse siga alguém.'
  end
end
